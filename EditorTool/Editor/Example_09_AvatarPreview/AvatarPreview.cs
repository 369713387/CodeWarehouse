...
namespace ToolKits
{
    public class AvatarPreview
    {
        ...
        {
            // 从跟踪列表中移除实例
            if (s_ActivePreviews != null)
            {
                s_ActivePreviews.Remove(this);
            }
            
            // 强制清理PreviewRenderUtility
        ...
        private void CleanupPreviewUtility()
        {
            if (m_PreviewUtility != null)
            {
                ...
                {
                    Debug.LogWarning($"Error during PreviewRenderUtility cleanup: {ex.Message}");
                }
                finally
                {
        ...
        public AvatarPreview(Animator previewObjectInScene, Motion objectOnSameAsset)
        {
            // 注册实例以便跟踪
            if (s_ActivePreviews == null)
                s_ActivePreviews = new List<AvatarPreview>();
        ...
        // 静态引用追踪所有活跃的PreviewRenderUtility实例
        private static List<AvatarPreview> s_ActivePreviews = new List<AvatarPreview>();
        ...
        private static void OnScriptsReloaded()
        {
            ...
            // 强制清理所有可能残留的PreviewRenderUtility
            // 在脚本重新编译时，静态列表会被重置，但我们仍然尝试清理
            EditorApplication.delayCall += ForceCleanupAllPreviewUtilities;
        }
        
        private static void ForceCleanupAllPreviewUtilities()
        {
            // 清理所有活跃的预览实例
            if (s_ActivePreviews != null)
            {
                var previewsToCleanup = new List<AvatarPreview>(s_ActivePreviews);
                foreach (var preview in previewsToCleanup)
                {
                    if (preview != null)
                    {
                        preview.CleanupPreviewUtility();
                    }
                }
                s_ActivePreviews.Clear();
            }
