...
namespace ToolKits
{
    ...
    public class ParticleSystemPreview : ObjectPreview
    {
        private class Styles
        {
            ...
            public GUIContent timeline = new GUIContent("Timeline", "Particle timeline scrubber"); // 添加时间轴样式
        ...
        private bool m_IsTimelineScrubbing = false; // 添加时间轴拖拽状态标记
        ...
        public override void OnPreviewSettings()
        {
            ...
            EditorGUI.BeginChangeCheck();
            float newTime = GUILayout.HorizontalSlider(m_RunningTime, 0f, kDuration, s_Styles.preSlider, s_Styles.preSliderThumb, GUILayout.MaxWidth(120f));

            if (EditorGUI.EndChangeCheck())
            {
                m_RunningTime = newTime;
                m_IsTimelineScrubbing = true;
                OnTimelineChanged();
            }

            // 检测时间轴拖拽结束
            if (m_IsTimelineScrubbing && Event.current.type == EventType.MouseUp)
            {
        ...
        private void InspectorUpdate()
        {
            ...
            if (m_Playing && !m_IsTimelineScrubbing)
            {
                m_RunningTime = Mathf.Clamp(m_RunningTime + (float)delta, 0f, kDuration);
                SimulateUpdate();
                //Debug.Log("自动播放，当前粒子时间: " + m_RunningTime);
            }
            // else if(m_IsTimelineScrubbing)
            // {
            //     m_RunningTime = Mathf.Clamp(m_RunningTime + (float)delta, 0f, kDuration);
            //     SimulateUpdate();
            //     Debug.Log("拖拽时间轴，当前粒子时间: " + m_RunningTime);
            // }
        }
